...
switch(Number(metodo)){
    ...
    case 3:
        ...
        break
    case 4:
        res = pontoCritico(separacao,-100,100)
        console.log(res)
        break
    case 5:
...
function printExpr(e) {
    for (let i = 0; i < e.length; i++) {
        if(!(e[i] instanceof Object) || operadores.includes(e[i])) continue
        if(e[i].variavel == null){
            e[i] = e[i].coeficiente
            continue
        }
        if(e[i].potencia[0]/e[i].potencia[2] == 1){
            e[i].potencia = null
            e[i].variavel = "x"
        }
        if(e[i].coeficiente[0]/e[i].coeficiente[2] == 1){
            e[i].coeficiente = null
        }
        if(e[i].coeficiente[2] == 1){
            e[i].coeficiente.splice(1,2)
        }
        // if(e[i].potencia == null){
        //     e[i] = e[i].coeficiente.join("") + e[i].variavel
        //     continue
        // }
        if(e[i].potencia[2] == 1){
...
function substituirX(exp,x){
    ...
    for(let i =0;i<obj.length;i++){
        ...
        if(obj[i].variavel == null){
            obj[i] = (obj[i].coeficiente[0]/obj[i].coeficiente[2])**(obj[i].potencia[0]/obj[i].potencia[2])
        }
        else{
            obj[i].variavel = x
            obj[i] = (obj[i].coeficiente[0]/obj[i].coeficiente[2])*(obj[i].variavel**(obj[i].potencia[0]/obj[i].potencia[2]))
        }
    }
    return calculoNormal(obj)    
}

//Função que calcula operções básicas, respeitando a ordem das operações mátemáticas
function calculoNormal(exp){
    let ops1 = ["^", "**"];
    let ops2 = ["*", "/", "%"];
    let ops3 = ["+", "-"];
...
function niveis(obj,ops){
    ...
    while(i < obj.length){
        ...
        if(ops.includes(elem)){
            let resultado = operacoes[elem](obj[i-1],obj[i+1])
...
function pontoCritico(exp,i,f)
{
    ...
    for(let i =0;i<raiz.length;i++){
        res.push([raiz[i],substituirX(exp,raiz[i])])
        derivada2 = tombo(tombo(exp))
        f2 = substituirX(derivada2,raiz[i])
        if(f2 === 0) res.push("Indeterminada")
        else if(f2 > 0) res.push("Minimo")
        else if(f2 < 0) res.push("Máximo")
    }
    return res
}
...
function acharTodasRaizes(obj, inicio, fim, passo = 0.1) {
    let raizes = [];
